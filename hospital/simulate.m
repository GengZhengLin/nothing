...
function update_I1(k)
global I1 i1 T1_new mu1_new u1;
k=k-1;
I1(:,k+1)=I1(:,k)+(1-u1)*i1(:,k)-(T1_new(:,k)+mu1_new(:,k)).*I1(:,k);
end
...
function update_O(k)
global O I1 mu1_new I2 mu2 b;
k=k-1;
O(:,k+1)=O(:,k)+I1(:,k).*mu1_new(:,k)+I2(:,k).*mu2-b*O(:,k);
end
...
if k<=T
    ...
    T1_new(:,k+1)=T1-M(:,k)./(N.*I1(:,k))*T1+M(:,k)./(N.*I1(:,k))*(1-nm)*yita;
end
end
